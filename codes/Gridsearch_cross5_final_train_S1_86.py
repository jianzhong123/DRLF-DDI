...
def train(model, train_data_loader, index, test_data_loader,loss_fn,  optimizer, n_epochs, device, scheduler=None):
    ...
    for i in range(1, n_epochs+1):
        ...
        with torch.no_grad():
            ...
            if best_f1 < test_f1:
                best_f1 = test_f1
                torch.save(model.state_dict(), 'DVFL_S1_ryu_cross' + str(index) + '.pkl')
                print('model saved')
                predlabls += 1
                truelabels += 1
                concatenated = np.concatenate(
                    ( truelabels[:, np.newaxis],predlabls[:, np.newaxis]),
                    axis=1)
                # 保存到CSV文件
                np.savetxt('DVFL_S1_ryu_results' + str(index) + '.csv', concatenated, delimiter=',')
