...
def train(model, train_data_loader, index,l,dim,c, test_data_loader,loss_fn,  optimizer, n_epochs, device, scheduler=None):
    ...
    for i in range(1, n_epochs+1):
        ...
        with torch.no_grad():
            ...
            if best_f1 < train_f1:
                best_f1 = train_f1
                torch.save(model.state_dict(), 'S1_DVFL_DDIMDL_cross' + str(dim)+str(l)+str(c)+str(index) + '.pkl')
                print('model saved')
                predlabls += 1
                truelabels += 1
...
if __name__ == '__main__':
    ...
    def run_model(l,dim,c):#params
        ...
        #for index in range(5, 0, -1):
            ...
            df_ddi_test = pd.read_csv('../data/Dengcross5datasets/Deng_S1/S1_DDIMDL_idx_test_cross_'+str(index)+'.csv',header=None)
            ...
            model = model_S0(device,256,2,2).to(device=device)
            # if index !=5:
            #     preindex=index+1
