...
def min_max_normalize(feature1, feature2):
    ...
    feature2_norm = (feature2 - min_val) / (max_val - min_val + 1e-8)  # 避免除 0
...
class model_S0(torch.nn.Module):
    def __init__(self,device,index,dim,l,head):#in_dim,hidden_dim,out_dim,
        ...
        self.mlp = nn.ModuleList([nn.Linear(1536,1536),#2560
                                  nn.ELU(),
                                  nn.Linear(1536, 86)
                                  ])
    ...
    def forward(self, drug1s, drug2s, train_graph):
        ...
        graph_emb1 = self.trans1(self.alignn, train_graph.edge_index)
        drug1_emb_graph = graph_emb1[drug1s.long()]
        drug2_emb_graph = graph_emb1[drug2s.long()]
